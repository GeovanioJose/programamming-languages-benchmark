...
#header
echo timestamp,total,psys,dram,cpu_temp > out/fannkuchredux_out.csv
echo runtime,cpu_usage,max_memory > out/runtime.csv


for (( i=1 ; i<=100 ; i++ ))
do
    echo fannkuchredux $i
    ...
    /usr/bin/time -ao out/runtime.csv -f "%e,%P,%M" python3 -OO fannkuchredux.python3-4.py 12
    ...
    echo $timestamp,$total,$psys,$dram,$cpu_temp_now >> out/fannkuchredux_out.csv
done
