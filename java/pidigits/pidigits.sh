...
#header
echo timestamp,total,psys,dram,cpu_temp > out/pidigits_out.csv
...
do
    ...
    /usr/bin/time -ao out/runtime.csv -f "%e,%P,%M" java -Djava.library.path=Include/java -cp . pidigits 10000
    ...
    echo $timestamp,$total,$psys,$dram,$cpu_temp_now >> out/pidigits_out.csv
done
