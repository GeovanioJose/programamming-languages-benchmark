...
#header
echo timestamp,total,psys,dram,cpu_temp > out/knucleotide_out.csv
...
do
    ...
    /usr/bin/time -ao out/runtime.csv -f "%e,%P,%M" java  -cp .:/opt/src/java-libs/fastutil-8.3.1.jar knucleotide 0 < ../../fasta-output.txt
    ...
    echo $timestamp,$total,$psys,$dram,$cpu_temp_now >> out/knucleotide_out.csv
done
